...
export const HackathonAttendanceCheckInPage = () => {
  ...
  const submit = async e => { e.preventDefault(); setSubmitting(true); setError(''); setMessage(''); try { const response = await attendanceAPI.hackathonCheckIn(sessionId, { pin }); setMessage(`${response.data.member.name}, your attendance is marked.`); setPin(''); } catch (err) { setError(err.response?.data?.message || 'Unable to mark attendance'); } finally { setSubmitting(false); } };
  ...
  return <div className="flex min-h-screen items-center justify-center bg-emerald-50 px-4 py-10"><main className="w-full max-w-lg rounded-lg border border-emerald-100 bg-white p-6 shadow-xl sm:p-9">
    ...
    <form onSubmit={submit} className="mt-6 space-y-4"><label className="block text-sm font-black"><FiKey className="mr-2 inline" /> Individual PIN<input value={pin} onChange={e => setPin(e.target.value.replace(/\D/g,'').slice(0,4))} inputMode="numeric" pattern="\d{4}" autoFocus required className="mt-2 w-full rounded-lg border px-4 py-4 text-center font-mono text-3xl tracking-widest outline-none focus:border-secondary" /></label><button disabled={!session?.qrEnabled || submitting} className="w-full rounded-lg bg-primary px-5 py-3 font-black text-white disabled:opacity-50">{submitting ? 'Checking...' : 'Mark My Attendance'}</button></form>
  </main></div>;
};
