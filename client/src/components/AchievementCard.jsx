      ...
      <div className={`grid ${featured ? 'lg:grid-cols-[1.05fr_0.95fr]' : ''}`}>
        <div className={`grid min-h-52 gap-1 bg-emerald-50 ${images.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
          {images.length ? images.slice(0, featured ? 4 : 2).map((image, index) => (
            <div key={image} className={`relative overflow-hidden ${index === 0 && images.length === 3 ? 'row-span-2' : ''}`}>
              <img src={resolveMediaUrl(image)} alt={`${achievement.title} ${index + 1}`} className="h-full min-h-52 w-full object-cover" loading="lazy" />
